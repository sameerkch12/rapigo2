...
module.exports.verifyEmail = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    return res.status(400).json({ message: "Invalid or expired verification token", error: err.message });
  }
...
module.exports.sendOtp = asyncHandler(async (req, res) => {
  ...
  await userModel.findOneAndUpdate(
    { phone },
    { phone, otp: otpResult.otp, otpExpires },
...
module.exports.verifyOtp = asyncHandler(async (req, res) => {
  ...
  const user = await userModel.findOne({ phone }).select("+otp +otpExpires");
  ...
  const isProfileComplete = Boolean(user.fullname?.firstname && user.email);
  ...
  return res.status(200).json({
    message: "OTP verified. Profile registration required for first-time user.",
    isNewUser: true,
    phone: user.phone,
  });
});

module.exports.registerPhoneUser = asyncHandler(async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(400).json(errors.array());
  }
  ...
  res.status(201).json({
    message: "User profile completed & registered successfully",
    isNewUser: false,
    token,
    user: {
      _id: user._id,
